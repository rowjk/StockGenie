...
* ~~已驗~~ 預約單開盤自動轉「已送出」（08:30 轉送顯示 X005A）；~~已驗~~ 成交後未成交清單自動移除。
* ~~已驗~~ 拒單紅色警示 toast（0050 @110 漲跌幅拒單實際跳出）。
* ~~已驗~~ 零股下單/成交/掛單（0050 盤中零股多筆）。
* 未驗（被動觀察即可）：部分成交時「已成交」欄與狀態顯示（可遇不可求）；委託紀錄 >10 筆 scroll；Boss Key 對新卡片遮蔽（順手按 Esc 確認）。
* v1.9.0 新功能待實測：下單面板市價顯示與漲跌幅攔截（下次下單時看一眼）。
