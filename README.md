...
v1.6 起新增「API 金鑰多組管理」與「Demo 演示模式」：可在設定頁維護多組永豐 API 金鑰／CA 憑證並一鍵安全切換（自動重啟 Shioaji 守護進程）；Demo 模式以前端攔截供應高擬真假數據（波動行情、模擬下單閉環、資產趨勢校準），可在不洩漏真實資產、甚至完全離線的狀態下對外演示。
...
* **版本號同步更新限制**：未來在進行任何系統功能升級、修補 bug 或修改程式碼時，**版本號必須同步跟著更新**（目前版本為 `v1.6.0`）。請在修改完成後，主動更新：
...
* **v1.6.0** - **API 金鑰多組管理與 Live Demo 演示模式**：
  * **API 憑證多組管理**：新增 `credentials.json` 多組設定檔儲存（自 `.env` 無縫升級、原子寫入、已被 `*.json` gitignore 規則排除）。設定頁可新增/編輯/切換/刪除設定檔；金鑰一律遮蔽傳輸（API Key 前 6 後 4、Secret/密碼固定長度），編輯時遮蔽欄位自動保留原值。
  * **切換自動重啟**：切換或變更啟用中設定時，後端以執行緒鎖安全終止並重啟 `shioaji.exe`（含 Port 8080 釋放等待），同步合併寫入 `.env`（保留非金鑰設定，供 `monitor.py` 取用）；前端顯示重啟鎖定遮罩並每秒輪詢，30 秒逾時提供強制解鎖。
  * **安全防護**：所有變更端點須通過 `PEA6` 二次安全驗證（403 拒絕）；禁止刪除最後一組與啟用中設定檔（400）；設定 API 完全不依賴 Shioaji 進程，金鑰填錯也不會鎖死系統。
  * **Demo 演示模式**：`smartFetch` 統一收口前端請求，啟用時全面攔截改以記憶體假數據回應（零實體 API 請求、可完全離線）：擬真餘額/額度/四檔持倉/交割款、自選股與美股 ±0.1~0.3% 隨機漫步波動、分時與 250 日 K 線（MA 可算）、12 個月假損益、假公告/除權息；90 天資產趨勢終值動態校準為「假餘額＋假庫存市值」。
  * **下單閉環模擬**：Demo 下單虛擬攔截，1~2 秒模擬成交並動態更新假庫存與假餘額；賣出僅限假庫存且不得超過持有量，買進受假餘額限制。
  * **DEMO 徽章與關閉防線**：啟用時 Header 顯示霓虹藍 `DEMO` 徽章；關閉 Demo（還原真實資產）須通過安全驗證。

### 📅 2026-06-10 (v1.5.0 至 v1.5.2)

* **v1.5.2** - **Code Review 問題修正**：
  * **TWSE 憑證 SSL Lock 修正**：補上全域 `_twse_needs_relaxed_ssl_lock` 鎖的宣告與調用，修正原先 `dashboard.py` 中鎖變數未定義的潛在 NameError 崩潰隱患。
  * **歷史資料清理邏輯修正**：將原先 `len(history_dict) > 1000` 且 Prune 至 365 天的邏輯，調整為 `len(history_dict) > 3000`（約 8 年的每日資料）才啟動清理，且清理方式改為依日期排序保留最新 3000 筆，避免手動導入大量歷史淨值數據時被自動 Prune 砍除的缺陷。同時修正了 `pruned_dict` 未定義 NameError 閃退 Bug。
