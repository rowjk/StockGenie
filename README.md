...
* **v1.13.0**（2026-09-17）：自選監控持久化與跨瀏覽器雙向自動同步——新增後端 `/api/watchlist` 與 `/api/us-watchlist` API，以線程安全鎖與原子寫入持久化至本機硬碟（`watchlist.json` 與 `us_watchlist.json`）；前端改於 `DOMContentLoaded` 階段即刻初始化自選清單，解除伺服器離線留白問題，並支援跨瀏覽器（如 Edge 轉 Chrome）及清除快取後的永久自動雙向同步；擴充單元測試驗證清單格式與邊界。
