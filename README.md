...
* **版本號同步更新限制**：未來在進行任何系統功能升級、修補 bug 或修改程式碼時，**版本號必須同步跟著更新**（目前版本為 `v1.4.5`）。請在修改完成後，主動更新：
...
### 📅 2026-06-10 (v1.4.0 至 v1.4.5)

* **v1.4.5** - **系統設定防護驗證機制**：
  * **進入設定問答攔截**：當使用者點選側邊欄的「系統設定」圖示時，將攔截直接進入，並彈出磨砂玻璃效果的「系統安全驗證」對話框。
  * **隨機洗牌問答選項**：使用者必須回答「機車+生命靈數」之問題，正確答案為 `PEA6`。系統會動態隨機生成 5 個符合 `[A-Z]{3}[0-9]{1}` 格式的錯誤答案，並將 6 個選項進行隨機洗牌，答對後才能進入，否則會彈出錯誤警告阻斷，提升系統在辦公室的防窺安全性。
